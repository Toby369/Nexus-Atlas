...
export const marketStateInfo = `So liest du das: Confidence kombiniert Datenabdeckung mit der Einigkeit der Faktoren. „Signal-Stärke“ zeigt, wie viele Faktoren überhaupt eine Richtung zeigen, „Konsens“, wie einig sich diese sind – so erkennst du, ob eine niedrige Confidence an fehlender Aussage oder an echtem Widerspruch liegt. Risk misst unabhängig davon die Fragilität der Lage (Warn-Muster, uneinige Zeitrahmen, Funding-Crowding, hohe Volatilität). MTF-Alignment zeigt die gewichtete Trend-Übereinstimmung über 1H/4H/1D als eine Zahl; Muster wie „Short Squeeze“ sind Interpretationshilfen, keine Handelssignale. Unter 35/100 Confidence wird ein berechnetes Bullisch/Bärisch bewusst als „Unklar / kein Zustand“ angezeigt – reine Anzeige-Entscheidung, der gespeicherte Zustand bleibt für Backtests unverändert. Die „MTF-Ampel“ darunter (22.09.2026) zeigt zusätzlich die Rohwerte pro Zeitrahmen: je ein Punkt für 15M/1H/4H/1D/1W, grün/rot = bestätigter Trend (ADX ≥25), gold = Trendbildung (ADX <25, Struktur aber schon eindeutig), grau = neutral, ausgegraut = keine Daten (1W bewusst immer, da noch keine Wochenkerzen gesammelt werden) – tippe/hover einen Punkt für den genauen Wert.
...
„Wird neu validiert“: Anders als der Setup-Score daneben (dreifach out-of-sample gegen echte Setup-Trefferquoten getestet) ist die 14-Faktoren-Formel hier bisher nicht gegen eine eigene Zielgrösse statistisch validiert. Institutionelle Praxis trennt Regime-Einschätzung von Trade-Signal-Scoring strikt (siehe docs/research/NEXUS-STRUKTUR-KONZEPT_2026-09-12.md, Abschnitt 5) – ein eigenes, vorregistriertes Gesamteinschätzung-Score-Protokoll ist als nächstes eigenständiges Vorhaben vorgesehen.`;

export const marketStateMatrixInfo = `So liest du das: Die sieben möglichen Regimes sind „Trendausweitung“ (bullisch/bärisch, wenn ADX ≥25 und Trendrichtung/Regressionssteigung übereinstimmen), „Trendbildung“ (dieselbe Übereinstimmung, aber ADX noch zwischen 20 und 25 – ein schwächerer, noch nicht vollständig bestätigter Trend, seit 21.09.2026 ergänzt), „Volatilitäts-Squeeze“ (niedriger ADX bei komprimierten Bollinger-Bändern – typische Phase vor einem Ausbruch), „Hohe Volatilität/Reversion“ (Volatilitätsspitze bei gleichzeitig weit vom Mittelwert entferntem Preis) oder „Unklar / kein Regime“ als bewusst nicht erfundenes Ergebnis, wenn die Signale nicht übereinstimmen oder Kerndaten fehlen. Das ist eine von der 14-Faktoren-„Gesamteinschätzung“ oben unabhängige, zusätzliche Einordnung, kein Ersatz dafür. Dieselbe Confidence-Sperre wie beim Gesamtzustand gilt auch hier: Liegt die Confidence der Gesamteinschätzung unter 35/100, werden die gerichteten Regimes (Trendausweitung UND Trendbildung) hier ebenfalls nicht als solche angezeigt, sondern als unklar gekennzeichnet. Zusätzlich wird angezeigt, wenn die Richtung dieser Regime-Einordnung und die Richtung des 14-Faktoren-Gesamtzustands übereinstimmen oder sich widersprechen (Engine-Divergenz) – nur sichtbar, wenn beide Engines eine eindeutige Richtung liefern und die Confidence-Sperre nicht bereits greift. Ein optionales „TradingView Context“-Badge zeigt das jüngste externe Signal (Pine-Script-Alert, per Webhook empfangen) der letzten 24 Stunden – rein informativ, fließt nicht in Score, Confidence oder Regime ein und wird nach 24 Stunden automatisch ausgeblendet. Zusätzlicher Hinweis „Kurzfristig bereits seitwärts“ (nur bei aktiver Trendausweitung sichtbar): die 1h-Engine kann nach einem abgeschlossenen Trendimpuls mehrere Stunden nachlaufen, da ADX/Regressionssteigung noch die vorherige Bewegung widerspiegeln – ein separater, unabhängiger Check auf den letzten 16 15-Minuten-Kerzen (4h) markiert das, wenn der ADX dort unter 20 liegt UND die Bollinger-Bänder eng sind (dieselben Schwellen wie beim Volatilitäts-Squeeze-Regime oben, nur auf kürzerer Zeitbasis). Ist oben ein Event-Anker gesetzt, zeigt „Seit Anker“ zusätzlich das Regime zum Anker-Zeitpunkt neben dem aktuellen (Phase 1 „Anchored Analytics“) – dieselbe Confidence-Sperre gilt dabei auch rückwirkend für den historischen Wert. Die fünf kleinen Punkte oben rechts neben dem Titel sind die „MTF-Ampel“ (22.09.2026): je ein Punkt für 15M/1H/4H/1D/1W, grün/rot = bestätigter Trend (ADX ≥25), gold = Trendbildung (ADX <25, aber Struktur schon eindeutig), grau = neutral/seitwärts, ausgegraut mit Rahmen = keine aktuellen Daten (1W bewusst immer, da noch keine Wochenkerzen gesammelt werden). Tippe/hover einen Punkt für den genauen Wert. Eigenständig von den fünf Feature-Säulen unten (die sind reine 1H-Werte) – hier geht es nur darum, ob sich Struktur/ADX über mehrere Zeitrahmen hinweg gleich oder unterschiedlich lesen.
...
export const vwapVectorInfo = `So liest du das: Fünf VWAP-Varianten (Tag/Woche/Monat/seit letztem Swing-Hoch/seit letztem Swing-Tief) zeigen den volumengewichteten Durchschnittspreis seit unterschiedlichen Ankerpunkten – Preis darüber deutet auf Käuferkontrolle seit diesem Anker, darunter auf Verkäuferkontrolle. Der EMA-Fächer (1H und 4H) ist unverändert aus den bestehenden Nexus-Kennzahlen übernommen, keine Neuberechnung. Die Fibonacci-Retracement-Stufen liegen zwischen dem jüngsten Swing-Hoch und -Tief – ein reines Rechengitter (0–100%), keine erkannte ATR-Erweiterung wie im Original-Skript (dessen Quellcode nicht einsehbar ist). Reine Entscheidungsunterstützung, kein automatisches Handelssignal.

So entsteht der Wert: Wochen-/Monats-/Swing-VWAP über get_anchored_vwap_summary(), Tag-/Swing-Hoch-/Swing-Tief-VWAP über die neue RPC get_vwap_since() (gleiche Arithmetik, anderer Anker) – Swing-Anker aus derselben Swing-Erkennung wie beim GUSS-Signal (Lookback 20). Bei jedem Seitenaufruf neu berechnet.`;

export const cvdFootprintInfo = `So liest du das: Zeigt die tatsächliche Käufer-/Verkäufer-Aggressivität INNERHALB jeder 1H-Kerze (aus echten 1-Minuten-Taker-Buy/Sell-Volumina) – ein positives Delta bedeutet mehr aggressive Käufe als Verkäufe in dieser Stunde. "Kumulativ" ist nur über das geladene Zeitfenster aufsummiert, kein Wert seit Symbol-Beginn – gedacht für Trend-/Divergenz-Einordnung, nicht als absolute Zahl. Eine Divergenz-Meldung erscheint, wenn der Preis ein neues Hoch/Tief markiert, der CVD-Wert an diesem Punkt aber NICHT mitzieht – eine Interpretationshilfe (wie bei RSI-/MACD-Divergenz), kein automatisches Handelssignal. WICHTIG: unterscheidet sich bewusst von "cvd_delta"/"cvd_trend" in der Marktzustand-Kachel – dort wird dieselbe Kennzahl AM NATIVEN INTERVALL selbst berechnet (kein Intrabar-Rollup aus 1-Minuten-Daten), also ein gröberes, anderes Konzept.

So entsteht der Wert: Alle 1-Minuten-Kerzen innerhalb der letzten 48 Stunden werden ihrer jeweiligen 1H-Kerze zugeordnet und dort aufsummiert (2×Taker-Buy-Volumen − Gesamtvolumen je Minute – echte Binance-Aggressor-Daten, keine Schätzung). Divergenz-Check über Preis-Pivots mit Lookback 10 (wie Mo's Original-Skript). Bei jedem Seitenaufruf neu berechnet.`;

// Momentum-Divergenz-Warnhinweis (Prototyp, 19.09.2026, siehe
// lib/momentumDivergence.ts) -- reine Anzeige-Ableitung, keine neue
// Berechnung/Query, keine Aenderung an compute-market-state.
