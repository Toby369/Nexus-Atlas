...
def _make_trade(i: int, equity_before: float, net_pnl: float) -> Trade:
    ...
    return Trade(
        ...
        exit_reason="trail_exit" if net_pnl > 0 else "sl",
...
def test_compute_bucket_metrics_hand_calculated():
    ...
    m = compute_bucket_metrics("15m", "LONG", 1, trades, initial_equity=100.0)
...
def test_compute_bucket_metrics_empty():
    m = compute_bucket_metrics("1h", "SHORT", 2, [], initial_equity=1000.0)
    assert m.trades == 0
...
def test_profit_factor_infinite_when_no_losses():
    ...
    m = compute_bucket_metrics("5m", "LONG", 3, trades, initial_equity=100.0)
