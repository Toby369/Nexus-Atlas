"""5m-Variante von frequency_stage_a.py -- identische Methodik (Vorfilter
nach Haeufigkeit, nicht nach Outcome), auf output/signal_windows_5m.csv.
MIN_N_PER_PHASE bleibt bei 500 (absolute Mindestgroesse fuer statistische
Power in Stufe B, keine Grid-relative Groesse -- bei ~3x mehr Kerzen am
5m-Grid wird dieser Schwellenwert ohnehin leichter erreicht)."""
import itertools
...
df = pd.read_csv("output/signal_windows_5m.csv")
...
freq.to_csv("output/frequency_stage_a_singles_5m.csv", index=False)
...
pair_candidates_df.to_csv("output/frequency_stage_a_pair_candidates_5m.csv", index=False)
