...
inkl. Fees). Gleiche Struktur wie in den anderen Projekten dieser Session,
angepasst auf die $-Werte statt %-Werte (fixe Marge statt %-Equity-Risiko).
"""
...
class BucketMetrics:
    ...
    total_return_pct: float  # relativ zum Start-Equity
...
def compute_bucket_metrics(timeframe: str, direction: str, trades: list[Trade], initial_equity: float) -> BucketMetrics:
    if not trades:
    ...
    return BucketMetrics(
        timeframe=timeframe, direction=direction, trades=len(trades), wins=len(wins), losses=len(losses),
        winrate_pct=winrate, profit_factor=profit_factor, avg_win_usdt=avg_win, avg_loss_usdt=avg_loss,
        total_pnl_usdt=total_pnl, total_return_pct=total_return, max_drawdown_pct=max_dd,
        sharpe_per_trade=sharpe, avg_r_multiple_gross=avg_r, final_equity_usdt=final_equity,
    )
