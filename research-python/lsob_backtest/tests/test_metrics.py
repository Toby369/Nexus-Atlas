...
def _make_trade(i: int, equity_before: float, net_pnl: float) -> Trade:
    ...
    return Trade(
        ...
        sl_price=95.0,
        tp_price=110.0,
        exit_price=100.0 + net_pnl / 20,  # nur fuer Vollstaendigkeit, nicht Teil der Pruefung
...
def test_compute_bucket_metrics_hand_calculated():
    ...
    m = compute_bucket_metrics("1h", "long", trades, initial_equity=100.0)
...
def test_compute_bucket_metrics_empty():
    m = compute_bucket_metrics("4h", "short", [], initial_equity=1000.0)
    assert m.trades == 0
